...
class Transformer(nn.Module):
    ...
    def forward(self, indices):
        ...
        return nn.functional.log_softmax(lineared, dim=-1), attn_maps
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, input_vecs):
        attn_output, _ = self.self_attn(input_vecs)
        out1 = self.norm1(attn_output + input_vecs)
        feedforward_output = self.feedforward(out1)
        out2 = self.norm2(feedforward_output + out1)
        return out2
...
def train_classifier(args, train, dev):
    vocab_size = 27 ; num_position = 20 ; d_model = 32
    d_internal = 129 ; num_classes = 3 ; num_layers = 2
    model = Transformer(
    ...
    for t in range(num_epochs):
        ...
        for ex_idx in ex_idxs:
            idx += 1
            if idx%100 == 0:
                print(f"Epoch {t}: {idx} / {len(ex_idxs)}")
            example = train[ex_idx]
            output, _ = model(example.input_tensor)
